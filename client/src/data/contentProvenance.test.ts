...
import { getContentProvenance, getContentProvenanceSummary } from "@shared/contentProvenance";

describe("content provenance", () => {
  it("classifica os materiais didáticos da escola como conteúdo próprio", () => {
  ...
  it("registra vídeo, documentação, fonte, licença e uso como referências externas", () => {
    const course = {
    ...
    const external = getContentProvenance(course).filter((item) => item.origin === "externo");
